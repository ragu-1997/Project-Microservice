node('master') {
    try {
    	...
    	stage('Publish') {
    		...
    		sh label: 'Copy dist/server.js', script: 'sudo cp -v ${WORKSPACE}/dist/server.js /opt/apps/WorkFlowService/DatabaseLogic/server.js'
            sh label: 'Copy Node Modules', script: 'sudo cp -R ${WORKSPACE}/node_modules /opt/apps/WorkFlowService/DatabaseLogic'
            sh label: 'Copy server.config.js', script: 'sudo cp -v ${WORKSPACE}/server.config.js /opt/apps/WorkFlowService/DatabaseLogic/server.config.js'
            sh label: 'Copy appversion.json', script: 'sudo cp -v ${WORKSPACE}/appversion.json /opt/apps/WorkFlowService/DatabaseLogic/appversion.json'
            deleteDir()
    	}
